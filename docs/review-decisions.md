...
| C4 | **`block` matching rules must be enforceable by the database**: allowed only with `method: normalized`/`exact`, materialised for the active immutable rule generation into a rule-FK `record_match_keys` table with a unique index written in the write transaction. A nonunique lookup table serves warn/group reads and pending backfill; old active enforcement remains until collision-free atomic activation. `fuzzy` rules may only `warn` (validated at `crm_matching_rule_set`). This also removes the `_n.*` shadow keys from `records.data` entirely (fixes R2 S5.2 structurally). |
...
| C7 | **Soft delete releases unique and matching keys**; restore recomputes current active/replacement rule keys and fails with `RESTORE_CONFLICT` + MRTR offer when a unique/active-block key was taken meanwhile. Matching rule ids are never restored from a stale delete snapshot. |
...
| M14 | `record_unique_keys` and matching-key tables index sha-256 of canonical normalized/exact JSON. Only `record_unique_keys` keeps its normalized value for the owning unique-attribute workflow; matching evidence is reconstructed from policy-safe record output and never stored in the key tables. |

...
| Identity | UOA is the sole authority for humans, orgs, teams. No local users, no local passwords, no profile mirrors beyond a non-authoritative name. | Identical — grounded in UOA's own `/llm` integration guide: delegations via the confidential token exchange, orgs/teams from the token's `org`/`active` claims, direct clients via the public OAuth profile. **[spec/uoa-integration.md](spec/uoa-integration.md)** is the contract. DeepCRM stores **no** user table. |
...
3. **Who else may call it?** **Decided (2026-08-23):** family products call via UOA confidential token exchange (app key + delegation + app context — no re-login, the ids arrive cryptographically); direct clients use UOA's public-client / MCP OAuth profile (`/oauth/*`, OAuth 2.1 + PKCE), same JWKS and iss/aud verification, behind `DEEPCRM_DIRECT_CLIENTS`. Contract: [spec/uoa-integration.md](spec/uoa-integration.md).
...
13. **Composite quantity values.** A value + unit + tolerance band that must travel together (manufacturing capability envelopes) has no type today — `number` plus a unit `text` can drift apart. `currency` is the one solved composite; generalise? (Round-two R19.)
14. **Suppression scope.** Suppression entries are per-team today (the tenant). Should an organisation-wide — or even instance-wide — suppression check exist, so an objection recorded by one team suppresses sends from every team? Legally attractive, tenancy-crossing by definition; needs its own design.
15. **Matrix as the family's event fabric.** Should DeepCRM's change-feed delivery (§6.5) grow a `matrix_room` target, and — the bigger question, owned by Nessie — should Nessie channels ever sit on a Matrix homeserver so products and customers' own agents share rooms? Not needed for v1; the delivery seam keeps the door open (§3.5).

> Default answers assumed by the docs and plans until you say otherwise: Q1 Prisma (+ `$queryRaw`), Q2 JSONB + change log, Q3 decided (UOA token exchange for products, UOA public OAuth profile for direct clients), Q4 copy now/extract later, Q5 tenant = org + team, Q6 agents call `crm_activity_log`, Q7 own Ledger key, Q8 MRTR only, Q9 generic tools only, Q5 decided (org+team), Q10 as stated, Q11/Q12 deferred, Q13 deferred, Q14 per-team for now, Q15 not in v1.

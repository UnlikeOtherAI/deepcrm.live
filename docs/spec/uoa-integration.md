...
| `act` | Upstream product chain, recorded verbatim into provenance and audit (auth-and-tenancy §1). |
...
There is **no** flat `team`, `role` or `tv` claim — earlier drafts of `auth-and-tenancy.md` inventing those are superseded by this table.

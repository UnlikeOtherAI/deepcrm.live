...
| `DEEPCRM_SECRET_KEYRING_B64` | — | required versioned keyring (`{active, keys}`); every 32-byte key supports AES-256-GCM, and the named `export` key is mandatory for export URL HMACs |
...
| `DEEPCRM_FILE_ACCESS_PUBLIC_URL` | `DEEPCRM_API_PUBLIC_URL` | public base URL for signed file access links returned by `crm_file_list` |

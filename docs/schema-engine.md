...
- `product` is current catalogue state. `line_item` is the historical commercial snapshot with copied SKU, description, quantity, unit price/cost, discount, tax, billing terms and total. Updating product price or cost never rewrites existing line items; corrections are explicit audited line-item updates.
- `line_item_product` unlinks on product deletion so historical snapshots keep their copied commercial fields. `line_item_deal` cascades when the commercial parent deal is deleted. Product-based dynamic segments use `linked_to` over `line_item_product` or saved views/lists over the same structured filter.

...
- Record writes, link/unlink operations, merge/restore/erasure paths that call
  ...
  Currency rollups only materialise when every contributing value carries the
  same ISO-4217 currency and, for fixed-currency targets, that exact currency;
  mixed-currency sums materialise `null` so agents must group or filter by
  currency explicitly rather than reading an unsafe total.
...
`standard_crm` (person, company, deal + relations + structural matching rules, now with tenant-customizable `lifecycle_stage` on person and company), `standard_sales` (lead object, lead-person/company/deal relations and a lead qualification pipeline), `standard_service` (ticket object, support relations, activity metadata and a ticket resolution pipeline), `standard_commerce` (product catalogue records, historical line-item snapshots, quote/subscription/invoice/payment/order document records, commercial party relations, lifecycle pipelines and rollup totals), and `system` (activity, note, task, their relation types — applied automatically only when a team is first provisioned). Templates are statically imported JSON modules (`resolveJsonModule`); no runtime filesystem read is permitted. `TemplateAdded` is exactly `{ objectTypes: number; attributes: number; relationTypes: number; pipelines: number; matchingRules: number }`. Template application is conservatively idempotent: independently absent object-type, stored/derived attribute, relation-type and pipeline slugs are added, but existing rows are never updated or archived. Primaries and matching rules are assigned only to object types created by that application. An existing row referenced by a new definition must be compatible or the transaction fails.

Both template paths run inside the caller's single transaction under the namespace-3 provisioning advisory lock. Trusted internal `applyTemplateBatch(tx, tenant, actor, slug)` performs six passes, in order — object-type shells with primaries unset; explicit relation types; stored attributes and compatible `record_reference` projection claims; derived attributes with dependency metadata and refresh jobs; pipelines with ordered stages; then primaries and matching rules — and returns `{ added: TemplateAdded }` without incrementing a version or writing an audit. Its shared internal relation primitive accepts the template's typed `isSystem` flag; the public schema mutation API still cannot set projection ownership. Public `applyTemplate(tx, tenant, actor, slug)` calls the batch seam, increments `schemaVersion` exactly once iff the sum of `added` is non-zero, and writes exactly one terminal `schema.template.apply` audit; a zero-addition call changes neither version nor audit state. First-contact tenancy instead calls the batch seam directly after default-policy seeding, increments `policyVersion` once and `schemaVersion` once iff the system template added anything, and writes exactly one terminal `tenant.provisioned` audit. It never also writes `schema.template.apply`.
...
Sales/service template notes:
- `lead` is not a person. It has structural qualification/disqualification fields, `owner (actor_reference role owner)`, `person/company/deal` record references, `external_ref` for sync, and the `lead_qualification` pipeline. `lead_person` is `many_to_one`, so multiple pursuits can involve the same person.
- `ticket` has subject, description, priority, category, source channel, open/close/SLA/first-response/resolution fields, `owner`/`assignee` actor-reference roles, `person/company/deal` references, `external_ref`, and the `ticket_resolution` pipeline. `ticket_product` points to any object until `standard_commerce` installs product metadata.
- Activity metadata is additive on the generic `activity` object: channel, outcome, thread reference, start/end/duration, plus restricted `transcript` and `attachment_refs`. These fields are normal attributes; policy and sensitivity redaction decide what timeline readers see.
- `product` is current catalogue state. `line_item` is the historical commercial snapshot with copied SKU, description, quantity, unit price/cost, discount, tax, billing terms and total. Updating product, quote, order, invoice, subscription or deal data never rewrites existing line items; corrections are explicit audited line-item updates.
- `quote`, `subscription`, `invoice`, `payment` and `order` are template-defined records with typed links to company/person/deal and to the relevant commercial parents. Payment records store provider names, provider references, status, amount and safe reconciliation ids only; card/bank data, secrets and raw provider payloads are rejected as unknown attributes rather than stored locally.
- `line_item_product` unlinks on product deletion so historical snapshots keep their copied commercial fields. `line_item_deal`, `line_item_quote`, `line_item_order`, `line_item_invoice` and `line_item_subscription` cascade when the commercial parent is deleted. Product/SKU-based dynamic segments use saved views or dynamic lists over visible `line_item` rows, optionally combined with `linked_to` filters for a specific product or parent; cross-tenant and hidden revenue rows are never segment members.
- Commerce rollups (`line_item_count`, `line_item_revenue_total`, `line_item_total`) are T59 derived attributes over active links. Currency totals materialise only for same-currency contributing line items; mixed-currency results are absent/null and callers group by currency explicitly.

...
- `scripts/lint-tenant-where.mjs`: scans `api/src/**/*.ts`, `worker/src/**/*.ts` and `packages/schema-engine/src/**/*.ts`; for every line matching `prisma\.(record|recordLink|recordChange|recordUniqueKey|recordMatchKey|recordSearch|objectType|attribute|relationType|matchingRule|list|listEntry|view|policyRule|policyBinding|approvalRequest|webhook|idempotencyReplay)\.(findMany|findFirst|findUnique|count|aggregate|updateMany|deleteMany)\(` the following 5 lines must contain `tenantWhere(` or a comment `// tenant-checked:`; otherwise print `file:line` and exit 1. Exits 0 when no files exist yet. (A runtime Prisma-extension backstop lands in T03.)
- `scripts/generate-mcp-docs.mjs`: placeholder that prints `docs:mcp not implemented until T26` and exits 0.
- `packages/db/prisma/schema.prisma` — **minimal placeholder** (generator + datasource blocks only, no models) so `prisma generate` succeeds before T03 replaces it.
- Package skeletons — each with `package.json` (`name`, `type: module`, `main: dist/index.js`, `types: dist/index.d.ts`, scripts `build: tsc -p tsconfig.build.json`, `typecheck: tsc --noEmit -p tsconfig.json`, `lint: eslint src --max-warnings=0`, `test: vitest run`), `tsconfig.json` (extends base, `include: ["src", "test"]`), `tsconfig.build.json` (extends base, `outDir: dist`, `rootDir: src`, `declaration: true`, `include: ["src"]`), `src/index.ts` exporting nothing yet (`export {}`):
  - `packages/schemas` (`@deepcrm/schemas`, deps: `zod@^3.25.76`, `undici@^6.21.0` — for `safeFetch`'s pinned Agent)
  - `packages/db` (`@deepcrm/db`, deps: `@prisma/client@^6.17.1`, `prisma@^6.17.1` — a runtime dep so the production image can run `migrate deploy`; extra scripts `prisma:generate: prisma generate`, `postinstall: prisma generate`)
...
- `errors.ts` — copy verbatim from `docs/spec/contracts.md` → `errors.ts` (the as-const `ErrorCode` map, `ErrorCodeValue`, `ErrorCodeSchema`, `ServiceError`, `isServiceError`, `NextHint`, `ErrorPayload`).
...
- `context.ts` — `PrincipalSchema` (zod) + inferred `Principal`, and the `ActorContext` type, exactly as auth-and-tenancy §2 (including `role`). This package is the single owner of `Principal` — `@deepcrm/mcp-inbound` imports it (never redefines it).
...
**Files:**
- Replace `packages/db/prisma/schema.prisma` — the exact contents of schema-engine §2 code block (copied files are exempt from the 500-line cap).
- Create `packages/db/src/client.ts`:
  ...
  ```
- Create `packages/db/src/tenant-where.ts` (**the one definition** — auth-and-tenancy §3):
  ```ts
  ...
  ```
- Create `packages/db/src/tenant-guard.ts` — Prisma client extension: on every query against a CRM model (the lint list), throw unless `where` contains both tenant columns or the call site is a seed helper (`NODE_ENV === 'test'` bypass env `DEEPCRM_TENANT_GUARD=off`); wired into `createDb`.
- Edit `packages/db/src/index.ts` — export `createDb`, `Db`, `tenantWhere`, `TenantRef`, and `export * from '@prisma/client'` for enums.
- Generate the migration: with the local Postgres running, `pnpm --filter @deepcrm/db exec prisma migrate dev --name init --create-only`. The schema copy already carries the two coherence columns — **`RecordLink.position Int?`** (nullable; null on direct links, contiguous 0-based on projected multi `record_reference` links) and **`RecordChange.resultingVersion Int`** (non-null on every row; record rows carry the record's `version` after the change is applied, `kind = schema` rows carry the current schema version — no null or sentinel anywhere) — so the generated SQL must contain both; verify with `rg "position|resulting_version" packages/db/prisma/migrations/*_init/migration.sql`. Then **append after the generated body** every raw statement listed as comments in schema-engine §2 (`records_data_gin`, `records_display_name_trgm`, `record_links_active_unique`, **`record_links_active_position_unique`** — the unique partial index on `(organization_id, team_id, from_record_id, relation_type_id, position) WHERE active_until IS NULL AND position IS NOT NULL` (comment in schema-engine §2 `RecordLink`), `record_search.tsv` generated column + gin, `record_search_embedding` hnsw, the `record_changes_schema_kind` check constraint; **not** `record_search_model` — that index remains T48's migration, not the init) and `CREATE EXTENSION IF NOT EXISTS vector; CREATE EXTENSION IF NOT EXISTS pg_trgm;` at the top (harmless duplicates of Prisma's own extension statements). Then `prisma migrate dev` (no `--name`; nothing drifted, it only applies).
...
DEEPCRM_API_PORT=5699 pnpm dev > /tmp/deepcrm-dev.log 2>&1 & DEV_PID=$!; sleep 6; curl -sf http://localhost:5699/health; kill $DEV_PID
```
curl prints `{"ok":true,…}`.
...
**Files:**
- Create `packages/mcp-inbound/src/principal.ts` — imports `Principal`/`PrincipalSchema` from `@deepcrm/schemas`; adds only `devPrincipal(requestId: string): Principal`, exactly the auth-and-tenancy §2 shape: `{ app:'dev', uoaUserId:'usr_dev', uoaOrgId:'org_dev', uoaTeamId:'team_dev', role:'owner', sourceDomain:'dev', product:'dev', actChain:[], agentId:'agent_dev', provenance:{ runId:'run_dev', toolCallId:'call_dev', requestId } }`. Provenance and `requestId` are **per-request** (the caller supplies them), never a stored constant; there is no `credentialEpoch` field anywhere in `Principal` (auth §2 is the whole contract). Unit test: `PrincipalSchema.parse(devPrincipal('req_x'))` passes and `provenance.requestId === 'req_x'`.
- Create `api/src/services/tenancy.ts` — `resolveTenant(deps, principal)`: advisory lock (namespace 3) on the external team id, upsert `organizations` by `externalOrgId` (name `Organisation ${id.slice(0,8)}`), upsert `teams` by `externalTeamId` with the **pairing check** (existing team under a different org ⇒ `TENANT_MISMATCH`, auth-and-tenancy §3); returns local ids + `schemaVersion` + `policyVersion` from the same query; in-process LRU cache (ids only) keyed `org:team` for 60 s.
...
**Files:**
- Create `packages/db/src/canonical-json.ts` — `canonicalJson(value)`: recursively sorted keys, NFC-normalised strings, arrays in order, no whitespace, numbers via `JSON.stringify`. Shared by the audit chain, `arguments_hash`, and the verifier.
- Create `packages/db/src/audit.ts` — `writeAudit(tx, entry: AuditEntryInput)`: `pg_advisory_xact_lock(5, hashtext(organizationId))` (taken immediately before the insert; no locks after), read the org's last `entry_hash`, compute exactly `sha256hex(utf8((prevHash ?? '') + '\n' + canonicalJson(entry)))` where `entry` excludes `id`/`prev_hash`/`entry_hash` (auth-and-tenancy §6 — note the parentheses: the whole concatenation is hashed), insert.
- Create `packages/db/src/audit.test.ts` (DB): two writes chain; concurrent writes from two connections do not fork (serialised by the lock). Create `scripts/verify-audit-chain.mjs`: reads `DATABASE_URL`, verifies **every** organisation's chain (exit 0 when none exist), exit 1 naming the first broken row; test: tamper a row's `reason` ⇒ exits 1.
- Create `packages/queue/src/index.ts` — `enqueue(db, { type, payload, organizationId?, teamId?, idempotencyKey?, visibleAt?, priority? })` (duplicate idempotency key returns the existing id); `claimNext(db, workerId, types[])`: `UPDATE queue_jobs SET status='running', locked_at=now(), locked_by=$2, attempts=attempts+1 WHERE id = (SELECT id FROM queue_jobs WHERE (status='queued' AND visible_at<=now() OR status='running' AND locked_at < now() - interval '10 minutes') AND type = ANY($1) ORDER BY priority DESC, created_at FOR UPDATE SKIP LOCKED LIMIT 1) RETURNING *` (the lease reclaims crashed workers); `complete`, `cancel`, `fail` (backoff `min(2^attempts, 60) min` until `max_attempts`, then `failed`), `progress`.
...
**Files:** create `.github/workflows/ci.yml` with jobs `lint`, `typecheck`, `test` (service `pgvector/pgvector:pg16`, `DATABASE_URL` env, runs `pnpm --filter @deepcrm/db exec prisma migrate deploy` then `pnpm test` then `node scripts/verify-audit-chain.mjs`), `build` (`docker build -f Dockerfile.app .` — **skip with `if: hashFiles('Dockerfile.app') != ''`** until T43). Node 24, pnpm 10.22 via `pnpm/action-setup`.

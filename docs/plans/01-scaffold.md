...
- Generate the migration: with the local Postgres running, `pnpm --filter @deepcrm/db exec prisma migrate dev --name init --create-only`, then **append after the generated body** every raw statement listed as comments in schema-engine §2 (`records_data_gin`, `records_display_name_trgm`, `record_links_active_unique`, `record_search.tsv` generated column + gin, `record_search_embedding` hnsw, the `record_changes_schema_kind` check constraint) and `CREATE EXTENSION IF NOT EXISTS vector; CREATE EXTENSION IF NOT EXISTS pg_trgm;` at the top (harmless duplicates of Prisma's own extension statements). Then `prisma migrate dev` (no `--name`; nothing drifted, it only applies).
...
**Files:**
- Create `packages/mcp-inbound/src/principal.ts` — imports `Principal`/`PrincipalSchema` from `@deepcrm/schemas`; adds only `devPrincipal(): Principal` = `{ app:'dev', uoaUserId:'usr_dev', uoaOrgId:'org_dev', uoaTeamId:'team_dev', role:'owner', credentialEpoch:0, agentId:'agent_dev', provenance:{ runId:'run_dev', toolCallId:'call_dev', requestId:'req_dev' } }`.

...
- T53/R24 owns verified UOA team re-parenting: enqueue `tenant.reparent`, revalidate the authoritative UOA organisation/team pairing before every batch, rewrite every tenant-scoped row's `organization_id` in bounded batches under the namespace-3 provisioning lock, audit the completed reconciliation, and return retryable `TENANT_REPARENTING` until complete. Earlier phases return `TENANT_MISMATCH` and must not reparent.

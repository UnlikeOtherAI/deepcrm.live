...
- Create `api/src/plugins/mcp-http.ts` — Fastify route `POST /mcp`: `authenticate` → 401 with `WWW-Authenticate` on failure; before `resolveTenant`, apply the authenticated per-app first-contact provisioning rate limit; then `resolveTenant` + `buildActorContext`; `const transport = new StreamableHTTPServerTransport({ sessionIdGenerator: undefined })` (stateless); `await server.connect(transport)`; `await transport.handleRequest(req.raw, reply.raw, req.body)`; `reply.hijack()`. `GET`/`DELETE /mcp` ⇒ 405. Add `_meta` cache hints to list results through a `server.server.setRequestHandler` wrapper or the SDK's list options — whichever the SDK 1.30 API offers; document the choice in a comment.

...
- **Pairing check + reconciliation (R24):** if the team row exists, its `organization_id` must equal the delegation's organisation. A mismatch on a **verified** token is not treated as an attack to be ratified by an operator — UOA is the hierarchy authority. DeepCRM enqueues an automatic `tenant.reparent` reconciliation job (rewrites `organization_id` across the team's rows in batches, audited, serialized under the provisioning lock) and answers `TENANT_REPARENTING` (retryable) until it completes. The stored FK is a cache with a heavy refresh, never a second authority.
